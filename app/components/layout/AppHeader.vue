<script setup lang="ts">
  ...
  const logoDark = computed(() => resolveExternal('/brand/logo/baldurdev-logo-dark.png'))
  const logoLight = computed(() => resolveExternal('/brand/logo/baldurdev-logo-light.png'))
...
<template>
  <UHeader v-model:open="menuOpen" data-site-header mode="slideover" :title="brandName" :to="homePath"
    :menu="headerMenu" :toggle="!menuOpen" :ui="headerUi">
    <template #title>
      <span class="block w-32 sm:w-36" :aria-label="brandName">
        <img :src="logoDark" alt="" width="800" height="200" class="h-auto w-full dark:hidden" />
        <img :src="logoLight" alt="" width="800" height="200" class="hidden h-auto w-full dark:block" />
      </span>
    </template>
    ...
    <template #right>
      <div class="hidden sm:flex items-center rounded-lg border border-border bg-bg-elevated p-0.5" role="group"
        ...
        <button v-for="opt in langOptions" :key="opt.code" type="button"
          class="min-h-9 min-w-9 px-2.5 text-xs font-medium rounded-md transition-colors" :class="locale === opt.code
    ...
    <template #content="{ close }">
      <div class="flex h-full flex-col bg-bg-elevated">
        <div class="flex items-center justify-between gap-3 border-b border-border px-5 sm:px-6 h-(--ui-header-height)">
          <NuxtLink :to="homePath" class="block w-32" :aria-label="brandName" @click="close?.()">
            <img :src="logoDark" alt="" width="800" height="200" class="h-auto w-full dark:hidden" />
            <img :src="logoLight" alt="" width="800" height="200" class="hidden h-auto w-full dark:block" />
          </NuxtLink>
          ...
          <div class="flex items-center gap-1.5">
            ...
            <UButton color="neutral" variant="ghost" icon="i-lucide-x" :aria-label="t('header.close')"
              class="min-h-11 min-w-11 rounded-lg" @click="close?.()" />
          </div>
        </div>
        ...
        <div class="flex flex-col gap-6 overflow-y-auto p-5 sm:p-6">
          <div class="flex items-center rounded-lg border border-border bg-bg p-0.5 w-fit" role="group"
            :aria-label="t('languages.switchTo')">
            <button v-for="opt in langOptions" :key="opt.code" type="button"
              class="min-h-9 px-3 py-1.5 text-xs font-medium rounded-md transition-colors" :class="locale === opt.code
                  ? 'bg-accent text-slate-900'
                  : 'text-text-muted hover:text-text hover:bg-surface-soft'
          ...
          <nav class="flex flex-col gap-1" :aria-label="t('header.description')">
            <NuxtLink v-for="link in navLinks" :key="link.label" :to="link.to"
              ...
              active-class="text-primary" @click="close?.()">
              {{ link.label }}
            </NuxtLink>
          </nav>
