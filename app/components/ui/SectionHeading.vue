...
<template>
  ...
  >
    <p
      v-if="eyebrow"
      class="text-xs font-semibold uppercase tracking-widest text-primary mb-3"
    >
      {{ eyebrow }}
    </p>
    <h2
      :id="id"
      class="text-3xl sm:text-4xl font-bold text-highlighted tracking-tight text-balance"
    >
      {{ title }}
    </h2>
    <p
      v-if="description"
      :class="[
        'mt-4 text-lg text-muted text-pretty',
        align === 'start' && 'md:max-w-prose'
      ]"
    >
      {{ description }}
    </p>
