<script setup lang="ts">
  ...
  onMounted(() => {
    ...
    if (!root) {
      console.error('[SkillsSection] No hay nodo de sección para GSAP')
      return
    }
    ...
    <div class="mx-auto max-w-7xl">
      ...
      <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <div v-for="category in categories" :key="category.key"
          class="skill-card group relative rounded-2xl border border-border bg-bg-elevated p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-accent/30 hover:shadow-lg">
