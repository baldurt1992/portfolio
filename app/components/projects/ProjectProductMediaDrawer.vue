...
<template>
  ...
  >
    <UButton
      size="sm"
      :color="highlightDetailsCta ? 'info' : 'neutral'"
      :variant="highlightDetailsCta ? 'soft' : 'outline'"
    ...
    <template #body>
      <div
        class="flex max-h-[min(85dvh,880px)] flex-col overflow-hidden lg:max-h-[min(90dvh,960px)] lg:flex-row"
      >
        <nav
          :aria-label="t('a11y.productMediaNav')"
          class="shrink-0 border-b border-default bg-elevated/20 lg:w-56 lg:border-e lg:border-b-0"
        >
          <ul
            class="flex gap-1 overflow-x-auto p-2 lg:flex lg:flex-col lg:overflow-y-auto lg:p-3"
          >
            <li v-for="(item, index) in items" :key="item.id" class="shrink-0 lg:w-full">
              <button
                type="button"
              ...
              >
                <span
                  ...
                  :class="index === activeIndex ? 'bg-primary text-inverted' : 'bg-default ring-1 ring-default'"
                >
                  {{ index + 1 }}
                </span>
                <span class="line-clamp-2 lg:line-clamp-3">{{ item.title }}</span>
              </button>
            </li>
          </ul>
        </nav>
        ...
        <div class="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto p-4 sm:p-5">
          ...
          <div
            class="mt-4 overflow-hidden rounded-xl bg-black ring-1 ring-default/60 shadow-sm"
          >
            <video
              :key="current.id"
